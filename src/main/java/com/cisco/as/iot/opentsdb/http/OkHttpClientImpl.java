...
public class OkHttpClientImpl extends HttpClientAbstract implements HttpClient {
	...
	protected Gson mapper = new GsonBuilder().create();
	...
	public OkHttpClientImpl(HttpRequestConfig config) {
		...
		client = new OkHttpClient.Builder().readTimeout(config.getReadTimeout(), TimeUnit.MILLISECONDS)
				.connectTimeout(config.getConnectTimeout(), TimeUnit.MILLISECONDS).connectionPool(connectionPool)
